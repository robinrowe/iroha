...
def doJavaBindings(os, buildType=Release) {
  ...
  sh """
    cmake \
      ...
      ${cmakeOptions}
  """
  def parallelismParam = (os == 'windows') ? '/MP' : '-j'
  sh "cmake --build build --target irohajava -- ${parallelismParam}${params.PARALLELISM}"
...
def doPythonBindings(os, buildType=Release) {
  ...
  """
  def parallelismParam = (os == 'windows') ? '/MP' : '-j'
  sh "cmake --build build --target irohapy -- ${parallelismParam}${params.PARALLELISM}"
...
def doAndroidBindings(abiVersion) {
  ...
  sh """
    ...
    git fetch --depth 1 origin ${env.GIT_LOCAL_BRANCH}; git checkout -t origin/${env.GIT_LOCAL_BRANCH})
  """
  sh """
    . /entrypoint.sh; \
